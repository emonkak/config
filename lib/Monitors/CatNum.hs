...
module Monitors.CatNum (CatNum(..)) where
...
data CatNumOpts = CatNumOpts { catNumDivier :: Float
                             , catNumSuffix :: String
                             }

defaultOpts :: CatNumOpts
defaultOpts = CatNumOpts { catNumDivier = 1
                         , catNumSuffix = ""
                         }

optDescriptions :: [OptDescr (CatNumOpts -> CatNumOpts)]
optDescriptions = [ Option "D" ["divier"] (ReqArg (\x o -> o { catNumDivier = parseFloat x }) "") ""
...
formatUnitNum digits n = let prefix = calcUnitPrefix n
