...
function! ku#file_mru#_append()  "{{{2
  ...
  if len(s:mru_files) > g:ku_file_mru_limit
    call remove(s:mru_files, -1)
  endif
  call s:save()
endfunction
...
function! s:save()  "{{{2
  call writefile(map(copy(s:mru_files), 'json_encode(v:val)'), g:ku_file_mru_file)
...
function! s:load()  "{{{2
  ...
  \ && s:mru_file_mtime != getftime(g:ku_file_mru_file)
    let s:mru_files = []
    for _ in readfile(g:ku_file_mru_file)[0:g:ku_file_mru_limit - 1]
      try
        let file = json_decode(_)
        if type(file) == type({})
          call insert(s:mru_files, file)
        endif
      endtry
    endfor
    let s:mru_file_mtime = getftime(g:ku_file_mru_file)
